...
const politicaDatos = ({ settings }) => {
  const data = settings.filter((setting) => setting.type === 'pagePrivacy');
  const pagePrivacy = data.reduce(
  ...
  return (
    <>
      <style jsx global>{`
        :root {
          --heroBgColor: ${pagePrivacy.heroBgColor.value};
          --bgColor: ${pagePrivacy.bgColor.value};
          --decorationColor: ${pagePrivacy.decorationColor.value};
        }
      `}</style>
